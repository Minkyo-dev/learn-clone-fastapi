...
class Users(Base, BaseMixin):
    __tablename__ = "notification_api.users"
    _table_args__ = {'schema': 'notification_api'}
...
class ApiKeys(Base, BaseMixin):
    __tablename__ = "notification_api.api_keys"
    _table_args__ = {'schema': 'notification_api'}
    ...
    user_id = Column(Integer, ForeignKey("notification_api.users.id"), nullable=False)
...
class ApiWhiteLists(Base, BaseMixin):
    __tablename__ = "notification_api.api_whitelists"
    _table_args__ = {'schema': 'notification_api'}

    id = Column(Integer, primary_key=True, index=True)
    ip_addr = Column(String(length=64), nullable=False)
    api_key_id = Column(Integer, ForeignKey("notification_api.api_keys.id"), nullable=False)
